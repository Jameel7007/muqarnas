...
export interface PlasterOptions {
  ...
  /** fired colour of the painted canopy */
...
export function plasterMaterial(opts: PlasterOptions = {}): MeshStandardNodeMaterial {
  ...
  const glazeCol = mix(color(opts.glazeCavity ?? 0x14565e), color(opts.glaze ?? 0x31b0a5), cavity);
  const wash = paint.mul(float(opts.glazeStrength ?? 0.75)).mul(broad.mul(0.12).add(1));
